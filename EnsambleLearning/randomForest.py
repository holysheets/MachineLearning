...
parser.add_argument( "--features", help="Number of features to choose from at gain",type=int)
...
class ID3Tree(object):
    ...
    def best_split_set(self,numberOfFeatures,featuresUsed):
        ...
        if numberOfAttributeOptions>numberOfFeatures:
            best_attribute=[0 for x in range(numberOfFeatures)]
        else:
            best_attribute=[0 for x in range(numberOfAttributeOptions)]
        potentialAttributeIndicies=[]
        ...
        for x in range(len(self.attributes)):
            ...
            for i in featuresUsed:
                if x==i:
                    found=True
            if not found:
                potentialAttributeIndicies.append(x)
        #indicies of potential attributes
        attributeOptions=numpy.random.choice(potentialAttributeIndicies,len(best_attribute),replace=False)
        idx=0
...
def train_set( data,numberOfFeatures):
    if args.car:
...
def randomForest(data, testData, T):
    numberOfFeatures=args.features
    ...
    for t in range(T):
        ...
        (tree,medians,mostCommonValues)=train_set(sampledData,numberOfFeatures)
...
if __name__ == "__main__":
    if not args.train:
        ...
        if not args.test:
            ...
            randomForest(trainData, testData, args.T)
